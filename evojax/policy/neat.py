...
from utils import *
from .utils.ngsa_sort import nsga_sort
...
class Neat:
    ...
    def initPop(self):
        ...
        for i in range(p['popSize']):
            ...
            newInd.conn[3, :] = (2 * (random.rand(1, nConn) - 0.5)) * p['ann_absWCap']
            newInd.conn[4, :] = random.rand(1, nConn) < p['prob_initEnable']
            newInd.express()
            newInd.birth = 0
    ...
    def probMoo(self):
        ...
        if self.p['alg_probMoo'] < random.rand():
...
def loadHyp(pFileName, printHyp=False):
    ...
    if hyp['alg_act'] == 0:
        ...
        hyp['ann_actRange'] = jnp.full_like(task.actRange, hyp['alg_act'])
