...
class Ind():
    ...
    def __init__(self, conn, node):
        ...
        Attributes:
          node    - (jnp_array) - node genes (see args)
          conn    - (jnp_array) - conn genes (see args)
          nInput  - (int)      - number of inputs
          nOutput - (int)      - number of outputs
          wMat    - (jnp_array) - weight matrix, one row and column for each node
                    [N X N]    - rows: connection from; cols: connection to
          wVec    - (jnp_array) - wMat as a flattened vector
                    [N**2 X 1]
          aVec    - (jnp_array) - activation function of each node (as int)
                    [N X 1]
    ...
    def createChild(self, p, innov, gen=0, mate=None):
        """Create new individual with this individual as a parent
          ...
          Args:
            ...
            innov  - (jnp_array) - innovation record
               [5 X nUniqueGenes]
               [0,:] == Innovation Number
               [1,:] == Source
               [2,:] == Destination
        ...
        Returns:
            ...
            innov  - (jnp_array) - updated innovation record
    ...
    def crossover(self, mate):
        """Combine genes of two individuals to produce new individual
          ...
          Args:
            parentA  - (Ind) - Fittest parent
              .conns - (jnp_array) - connection genes
                       [5 X nUniqueGenes]
        ...
        bGenes = jax.random.uniform(key, (len(matching),)) < bProb
        child.conn = child.conn.at[3, IA[bGenes]].set(parentB.conn[3, IB[bGenes]])
    ...
    def mutate(self, p, innov=None, gen=None):
        ...
        key = jax.random.PRNGKey(0)
        reenabled = jax.random.uniform(key, (len(disabled),)) < p['prob_enable']
        connG = connG.at[4, disabled].set(reenabled)

        # - Weight mutation
        # [Canonical NEAT: 10% of weights are fully random...but seriously?]
        key, subkey = jax.random.split(key)
        mutatedWeights = jax.random.uniform(subkey, (nConn,)) < p['prob_mutConn']  # Choose weights to mutate
        weightChange = mutatedWeights * jax.random.normal(subkey, (nConn,)) * p['ann_mutSigma']
        connG = connG.at[3, :].add(weightChange)
        ...
        key, subkey = jax.random.split(key)
        if (jax.random.uniform(subkey) < p['prob_addNode']) and jnp.any(connG[4, :] == 1):
    ...
    def mutAddConn(self, connG, nodeG, innov, gen, p):
        ...
        order, wMat = get_node_order(nodeG, connG)  # Topological Sort of Network
