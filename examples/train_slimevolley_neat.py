...
loses all five lives, or after 3000 ts has passed. An agent receives
...
def main(config):
    log_dir = './log/slimevolley-NEAT'
    if not os.path.exists(log_dir):
        os.makedirs(log_dir, exist_ok=True)
    logger = util.create_logger(
        name='SlimeVolley-NEAT', log_dir=log_dir, debug=config.debug)
